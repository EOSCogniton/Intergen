...
rep = 0.57; % Masse sur l'essieu arri?re en statique (%)
...
% Prise en compte du glissement, donn?es pneu simplifi?e : IA = 0, SA = 0, Load = 250 lb et p = 8psi
% coeff_piste = 1; % Correction du coeff d'adh?rence en fonction de la piste
...
% Mesure Optimus ? Chamb?ry le 1/05/19
...
%         Sl = [Sl, 0];
        ...
        C_ar = rep*m*g+m*a*h_g/emp; % Charge sur l'essieu arri?re avec prise en compte du transfert de masse
        if C_ar > m*g % Cas ou les roues avant se soulevent
            C_ar = m*g;
        end
        c_trans_ar = coeff_adh*C_ar*D_roue/2; % Couple maximum transmissible
        
            if c_k > c_trans_ar/10 % Risque de d?passement des capacit?s du pneu
                Adh = [Adh,1];
